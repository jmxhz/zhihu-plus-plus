...
- Sole fork Release asset: `zhihu++-lite-arm64-v8a.apk`.
...
- Preserve the fork release boundary even when upstream changes CI: `assembleLiteRelease` only, Gradle ABI split restricted to `arm64-v8a`, and exactly one Release asset named `zhihu++-lite-arm64-v8a.apk`.
...
4. Before triggering CI, verify `build.yml` and its reusable workflow build only `assembleLiteRelease`, do not contain desktop or Full build jobs, and publish only `zhihu++-lite-arm64-v8a.apk`.
...
6. On success, require the fork Release asset set to equal exactly `zhihu++-lite-arm64-v8a.apk`; set its title to `<tag> 自定义版` without discarding generated notes.
...
3. Wait for completion and verify the Release contains exactly `zhihu++-lite-arm64-v8a.apk`. Never delete/recreate the tag merely to retrigger CI.
